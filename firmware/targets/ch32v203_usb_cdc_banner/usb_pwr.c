...
/* Suspend disabled: Suspend() has a blocking while(ISTR_RESET==0) that
 * deadlocks when the xHCI fails enumeration and never sends RESET.
 * Keep FALSE so the retry loop in main() can recover via USB_Port_Set. */
