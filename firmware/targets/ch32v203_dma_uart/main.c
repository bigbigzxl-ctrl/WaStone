/* ch32v203_dma_uart — Dual DMA: DMA1 Ch4→USART1_TX + DMA1 Ch5→USART1_RX loopback
 *
 * TX: DMA1 Ch4 = USART1_TX (memory→peripheral)
 * RX: DMA1 Ch5 = USART1_RX (peripheral→memory)
 * Both DMAs start together; compare rx_buf to tx_buf after both TCs.
 * Wiring: PA9 → PA10 (same wire as uart_loopback Stage 1)
 *
 * Single-byte polling fails with overrun when TX DMA sends all bytes faster
 * than the polling loop can consume them from RX DR. Dual DMA avoids this.
 */
