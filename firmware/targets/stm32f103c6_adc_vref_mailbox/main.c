...
#define ERR_ADC_TIMEOUT 0x61u
#define ERR_SAMPLE_LOW  0x62u
#define ERR_SAMPLE_HIGH 0x63u
...
static void adc1_init_internal(uint32_t channel)
{
...
int main(void)
{
    ...
    {
        uint16_t avg = (uint16_t)(sum / 8u);
        AEL_MAILBOX->detail0 = avg;
        if (avg < 1000u) {
            ael_mailbox_fail(ERR_SAMPLE_LOW, avg);
            while (1) {}
        }
        if (avg > 2200u) {
            ael_mailbox_fail(ERR_SAMPLE_HIGH, avg);
            while (1) {}
        }
    }
