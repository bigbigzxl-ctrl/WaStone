/* ch32v305_wire_check — GPIO continuity check for Stage 2 loopback wiring
 *
 * Checks 4 wire pairs (drive HIGH+LOW, read back):
 *   PA1 (out) ↔ PA2 (in)    bit0  GPIO / EXTI loopback wire
 ...
 * detail0_increment on PASS: tick increments every ~1s.
 */
...
{
    /* ~96 MHz: ~96 cycles/us */
    for (uint32_t i = 0; i < us * 96; i++) __asm__("nop");
}
...
int main(void)
{
     ...
     * PA1, PA7, PA8, PA9 → Output Push-Pull 2MHz (bits = 0x2)
     * PA0, PA2, PA6, PA10 → Input with pull-down (bits = 0x8, ODR=0)
     *
     ...
     * PA1(out PP):   bits[7:4]   = 0x2
     * PA2(in pull):  bits[11:8]  = 0x8
     * PA6(in pull):  bits[27:24] = 0x8
     * PA7(out PP):   bits[31:28] = 0x2
     */
    uint32_t cfglr = GPIOA->CFGLR;
    cfglr &= ~((0xFu << 0) | (0xFu << 4) | (0xFu << 8) | (0xFu << 24) | (0xFu << 28));
    cfglr |=  ((0x8u << 0) | (0x2u << 4) | (0x8u << 8) | (0x8u << 24) | (0x2u << 28));
    GPIOA->CFGLR = cfglr;
    ...
    /* ODR = 0 for all pull-down inputs */
    GPIOA->OUTDR &= ~((1u << 0) | (1u << 2) | (1u << 6) | (1u << 10));

    delay_us(50);

    uint32_t fail_mask = 0;

    if (check_wire_pa(1, 2))   fail_mask |= (1u << 0);   /* PA1↔PA2 */
    ...
    uint32_t tick = 0;
    while (1) {
        /* On PASS: increment detail0 for detail0_increment liveness check.
