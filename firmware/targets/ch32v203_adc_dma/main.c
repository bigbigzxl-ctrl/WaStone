/* ch32v203_adc_dma — ADC1 continuous + DMA1_Ch1 buffer fill
 *
 * ADC1 in continuous scan mode, DMA1_Ch1 reads ADC1->RDATAR → buf[16].
 * Channel: Vrefint (ch17) — no external pin needed.
 * Expected: ~1.2V at 3.3V supply → 12-bit raw ≈ 1489; accept 800..1900.
 *
...
int main(void)
{
    ...
    for (int i = 0; i < BUF_LEN; i++) {
        if (adc_buf[i] < 800 || adc_buf[i] > 1900) bad++;
    }

    if (bad) {
        ael_mailbox_fail(2, adc_buf[0]);
    } else {
        ael_mailbox_pass();
        AEL_MAILBOX->detail0 = adc_buf[0];
    }
