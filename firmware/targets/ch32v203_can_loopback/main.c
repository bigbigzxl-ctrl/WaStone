/* ch32v203_can_loopback — CAN1 LoopBack self-test (no external transceiver needed)
 *
 * GPIO remap: GPIO_Remap1_CAN1 → PB8=CAN_RX, PB9=CAN_TX
 * Mode: CAN_Mode_LoopBack (internal loopback, no wire needed)
 * Baud: ~187.5 kbps @ APB1=36MHz: tsjw=1, tbs1=6, tbs2=5, brp=16
 * Filter: pass-all (mask=0), FIFO0
 * Send 8-byte payload, receive via FIFO0, verify byte-by-byte.
 */
...
static void CAN1_LoopBack_Init(void)
{
    ...
    CAN_InitStructure.CAN_Prescaler = 16;   /* ~187.5 kbps @ 36MHz APB1 */
    CAN_Init(CAN1, &CAN_InitStructure);

    /* Pass-all filter → FIFO0 */
...
int main(void)
{
    ...
    txMsg.Data[2] = 0x01; txMsg.Data[3] = 0xFF;
